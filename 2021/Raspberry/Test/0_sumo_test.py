...
# 영역 위치 판별 of 특정 hsv (카메라 변수, 카메라 마스크 변수, 색) + 원 생성
def state_decision(front_Camera_def, front_Camera_mask_def, color):
    # 연결된 객체 단위 분석 of 특정 hsv
    number, img_label, stats, centroids = cv2.connectedComponentsWithStats(front_Camera_mask_def)
    # print("number : ", number) # 확인용 코드
    # print("stats : ", stats) # 확인용 코드
    # print("centroids : ", centroids) # 확인용 코드
    count = 0

    # 연결된 객체 단위 저장 of 특정 hsv
...
while True:
    ...
    # hsv 색별 마스크 변환
    front_Camera_red_mask = masking(front_Camera_hsv, 171, 40)
    front_Camera_blue_mask = masking(front_Camera_hsv, 108, 100)
    front_Camera_green_mask = masking(front_Camera_hsv, 59, 60)

    # hsv 색별 state 종합 후 최종 결정 + 원 생성
    ...
    # - 초록 영역 크기 작게 감지 >> 초록 영역 위치만 보냄 + 원 생성
    if Green_state == 'l' or Green_state == 'r' or Green_state == 'm':
        Final_state = Green_state
    # - 초록 영역 크기 크게 감지 & 영역 감지 >> 빨 or 파 영역 위치만 보냄 + 원 생성 >> mbed에서 Green_state == 'GO'일 경우 Red_state와 Blue_state만으로 판단
    elif Green_state == 'GO':
        Red_state = state_decision(front_Camera, front_Camera_red_mask, 'red')
        Blue_state = state_decision(front_Camera, front_Camera_blue_mask, 'blue')

        # - 빨파 영역 감지 (빨강 1순위, 파랑 2순위, 영역 없음 3순위)
