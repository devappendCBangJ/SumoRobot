...
float gyroBias[3] = {-1.092, -0.435, -0.351}, accelBias[3] = {-0.060, 0.028, -0.039}; // Bias corrections for gyro and accelerometer
...
// Output scaled accelerometer biases for manual subtraction in the main program
   ...
   pc.printf("%7.3f,  %7.3f  , %7.3f, %7.3f,  %7.3f  , %7.3f\n", dest1[0],dest1[1],dest1[2],dest2[0],dest2[1],dest2[2]);
}

// void calibrateMPU9250(float * dest1, float * dest2)
// {  
//   uint8_t data[12]; // data array to hold accelerometer and gyro x, y, z, data
//   uint16_t ii, sample_count=100;
//   int32_t gyro_bias[3] = {0, 0, 0}, accel_bias[3] = {0, 0, 0};
//   uint8_t rawData[6] = {0, 0, 0, 0, 0, 0};

//   for( int ii = 0; ii < sample_count; ii++) { // get average current values of gyro and acclerometer

//     readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]); // Read the six raw data registers into data array
//     accel_bias[0] += (int16_t)(((int16_t)rawData[0] << 8) | rawData[1]) ; // Turn the MSB and LSB into a signed 16-bit value
//     accel_bias[1] += (int16_t)(((int16_t)rawData[2] << 8) | rawData[3]) ;
//     accel_bias[2] += (int16_t)(((int16_t)rawData[4] << 8) | rawData[5]) ;
...
//   }
//     accel_bias[0] /= (int32_t) sample_count; // Normalize sums to get average count biases
